local TARGET_NPC_ID = 731        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 208     -- set to nil for kill-only
local MAX_LEVEL = 40
local ACH_ID = "KingBangalash"
...
function KingBangalash_Kill(destGUID)
  RestoreFromProgress()
...
function KingBangalash_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function KingBangalash_IsCompleted()
