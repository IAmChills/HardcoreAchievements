...
local REQUIRED_QUEST_ID = 1393     -- set to nil for kill-only
local MAX_LEVEL = 39
local ACH_ID = "GalensEscape"
...
function GalensEscape_Kill(destGUID)
  RestoreFromProgress()
...
function GalensEscape_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function GalensEscape_IsCompleted()
