...
local REQUIRED_QUEST_ID = 3601     -- set to nil for kill-only
local MAX_LEVEL = 52
local ACH_ID = "KimJaelIndeed"
...
function KimJaelIndeed_Kill(destGUID)
  RestoreFromProgress()
...
function KimJaelIndeed_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function KimJaelIndeed_IsCompleted()
