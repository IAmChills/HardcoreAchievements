local TARGET_NPC_ID = 14621        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 7701     -- set to nil for kill-only
local MAX_LEVEL = 57
local ACH_ID = "OverseerMaltorius"
...
function OverseerMaltorius_Kill(destGUID)
  RestoreFromProgress()
...
function OverseerMaltorius_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function OverseerMaltorius_IsCompleted()
