local TARGET_NPC_ID = 3986        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 1017     -- set to nil for kill-only
local MAX_LEVEL = 24
local ACH_ID = "Foulborne"
...
function Foulborne_Kill(destGUID)
  RestoreFromProgress()
...
function Foulborne_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function Foulborne_IsCompleted()
