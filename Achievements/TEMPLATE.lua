local TARGET_NPC_ID = 12345        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 12345     -- set to nil for kill-only
local MAX_LEVEL = XX
local ACH_ID = "PLACEHOLDER"
local FACTION = "Alliance Horde Neutral"
local RACE = nil
local CLASS = nil
...
function PLACEHOLDER_Kill(destGUID)
  RestoreFromProgress()
...
function PLACEHOLDER_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function PLACEHOLDER_IsCompleted()
