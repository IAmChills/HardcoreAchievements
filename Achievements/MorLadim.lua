local TARGET_NPC_ID = 522        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 228     -- set to nil for kill-only
local MAX_LEVEL = 34
local ACH_ID = "MorLadim"
...
function MorLadim_Kill(destGUID)
  RestoreFromProgress()
...
function MorLadim_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
