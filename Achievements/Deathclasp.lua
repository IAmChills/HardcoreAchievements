local TARGET_NPC_ID = 15196        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 8283     -- set to nil for kill-only
local MAX_LEVEL = 60
local ACH_ID = "Deathclasp"
...
function Deathclasp_Kill(destGUID)
  RestoreFromProgress()
...
function Deathclasp_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function Deathclasp_IsCompleted()
