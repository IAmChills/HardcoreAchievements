...
local REQUIRED_QUEST_ID = 2681     -- set to nil for kill-only
local MAX_LEVEL = 52
local ACH_ID = "StonesThatBindUs"
...
function StonesThatBindUs_Kill(destGUID)
  RestoreFromProgress()
...
function StonesThatBindUs_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function StonesThatBindUs_IsCompleted()
