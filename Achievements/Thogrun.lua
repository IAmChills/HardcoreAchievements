local TARGET_NPC_ID = 2944        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 732     -- set to nil for kill-only
local MAX_LEVEL = 41
local ACH_ID = "Thogrun"
local FACTION = "Alliance Horde Nuetral"
local RACE = nil
local CLASS = nil
...
function Thogrun_Kill(destGUID)
  RestoreFromProgress()
...
function Thogrun_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
