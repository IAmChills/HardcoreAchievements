local TARGET_NPC_ID = 10738        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 5121     -- set to nil for kill-only
local MAX_LEVEL = 60
local ACH_ID = "HighChiefWinterfall"
...
function HighChiefWinterfall_Kill(destGUID)
  RestoreFromProgress()
...
function HighChiefWinterfall_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function HighChiefWinterfall_IsCompleted()
