local TARGET_NPC_ID = nil        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 731     -- set to nil for kill-only
local MAX_LEVEL = 20
local ACH_ID = "AbsentMindedProspector"
...
function AbsentMindedProspector_Kill(destGUID)
  RestoreFromProgress()
...
function AbsentMindedProspector_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function AbsentMindedProspector_IsCompleted()
