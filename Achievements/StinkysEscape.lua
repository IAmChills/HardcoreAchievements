...
local REQUIRED_QUEST_ID = 1270     -- set to nil for kill-only
local MAX_LEVEL = 35
local ACH_ID = "StinkysEscape"
...
function StinkysEscape_Kill(destGUID)
  RestoreFromProgress()
...
function StinkysEscape_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function StinkysEscape_IsCompleted()
