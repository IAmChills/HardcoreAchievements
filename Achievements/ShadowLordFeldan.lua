local TARGET_NPC_ID = 9517        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 5242     -- set to nil for kill-only
local MAX_LEVEL = 56
local ACH_ID = "ShadowLordFeldan"
...
function ShadowLordFeldan_Kill(destGUID)
  RestoreFromProgress()
...
function ShadowLordFeldan_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function ShadowLordFeldan_IsCompleted()
