local TARGET_NPC_ID = 11886        -- set to nil for quest-only
local REQUIRED_QUEST_ID = 5781     -- set to nil for kill-only
local MAX_LEVEL = 58
local ACH_ID = "MercutioFilthgorger"
...
function MercutioFilthgorger_Kill(destGUID)
  RestoreFromProgress()
...
function MercutioFilthgorger_Quest(questID)
  RestoreFromProgress()
  if state.completed or not BelowMax() then return false end
...
function MercutioFilthgorger_IsCompleted()
